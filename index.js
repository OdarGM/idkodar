...
client.on('ready', () => {
client.user.setPresence({game: {name: ` 77 servers - 645 users.`, type: 'WATCHING'}});
...
if(command === "dict") {
  ...
  if(command === "kick") {
    ...
    if(tokick.id === "291221132256870400") return message.channel.send("No")
  ...
  if(command === "ban") {
   ...
   if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("Sorry, you don't have permissions to use this!");
    ...
    if(Fopban.id === "291221132256870400") return message.channel.send("No")
...
Music.start(client, {
  prefix: ">",
  botOwner: 'REEEEEEEEEEEEEEEEEEEEE',
  youtubeKey: 'REEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE'
});
if (command === 'char') {
  const request = require("request")
  const cheerio = require('cheerio');
